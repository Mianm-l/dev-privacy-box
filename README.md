...
### 3. Cron 表达式可视化与执行预测 (Cron Visualizer)
- **自然语言翻译**：自动将 5 段/6 段 Cron 表达式翻译为人类可读的中文语句（如“每周一到周五 09:30 执行”）。
- **精准时间推算**：基于用户当前系统时区，毫秒级推算出未来 5 次的准确执行时间点与倒计时。
- **高频预设卡片**：内置每分钟、整点、每天凌晨、工作日上午等常用快捷模版。

### 4. JWT 纯离线透视与过期分析器 (JWT Inspector)
- **100% 离线解密**：完全在浏览器内存中解析 Header 与 Payload，绝不上报业务 Token。
- **过期倒计时**：自动将 `exp` / `iat` 转换为本地时间，并计算剩余有效期或过期时长。

### 5. 生产敏感数据批量脱敏助手 (Data Masker)
